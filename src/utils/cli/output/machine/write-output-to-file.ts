...
export function writeOutputToFile(
  ...
  if (!directoryExists) {
    return Result.err({
      _tag: "OutputDirectoryMissing",
      message: `No pude escribir la salida en ${outputPath}: el directorio ${outputDirectory} no existe.`,
    });
  }
  ...
  return Result.err({
    ...
    cause: outputWrite.error,
    message: `No pude escribir la salida en ${outputPath}.`,
  });
}
