 ...
 *
 * ### Prioridad
 * Primero parseo sintactico de `node:util`, luego path/help, despues presets/formato/adopcion, y al final exclusiones entre `--adopt`, `--verify`, `--resume-last` y `--reset-state`.
 *
 * ### Ejemplo
 * ```ts
 * parseCliArguments(["src", "--preset", "package", "--adopt", "10", "--yes"]);
 * // -> { ok: true, value: { path: "src", preset: "package", adoptPercent: 10, forceNonInteractive: true } }
 * parseCliArguments(["src", "--adopt", "10", "--verify", "seed"]);
 * // -> error de uso
 * ```
 */
export function parseCliArguments(args: readonly string[]): CliParseResult {
  ...
  return {
    ...
    value: {
      ...
      output: parsed.value.values.output ?? null,
      path,
      preset,
      rawPreset,
      resetState,
