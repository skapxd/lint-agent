...
export const noAnonymousCondition: RuleModule = {
  meta: {
    ...
    messages: {
      anonymousCondition:
        "La condicion sin nombre: este if evalua un computo cuyo significado el lector debe deducir. Extraelo a una const con nombre que diga QUE DECIDE el if, no que compara el codigo: `const esArchivoExento = matchesAnyGlob(archivo, patrones); if (esArchivoExento) ...`. El nombre debe tener significado: `is/has/needs/lacks/exceeds/reached` + el concepto del dominio (`reachedProjectRoot`, `exceedsStateBudget`). Cumplir la regla con un nombre vacio la traiciona: nada de sufijos Rule/Condition mecanicos, isNot mecanico, sufijos numerados (`isResult2`), ni nombres-AST concatenados a maquina (ruta del nodo + sufijo Rule) sin una firma type predicate que lo justifique. Un type predicate descriptivo largo como `isPublicClassMethod` es legitimo: lo que sobra es la concatenacion mecanica, no la longitud. La extraccion conserva el narrowing (TS 4.4+). Lo ya-nombrado no se extrae: variables (`isReady`), accesos como `result.ok` (hasta {{maxMemberDepth}} saltos), sus negaciones (`!result.ok`), comparaciones con literal booleano o nullish (`x.ok === false`, `x == null`) y type guards que la firma demuestra (`x is T`).",
    },
    schema: [
      {
