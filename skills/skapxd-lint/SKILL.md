...
Ejecuta el paquete publicado, anclado al major `@4`:

```bash
npx @skapxd/eslint-opinionated@4 <path> --yes --format toon
```

Anclado a major `@4` (no `@latest` mutable): recibes parches y minors sin saltar a un major que pueda romper o estar comprometido. El paquete se publica en npm con provenance (procedencia verificable). Para adopcion permanente, instala el paquete como devDependency (version fija + lockfile con integridad) y corre el bin `skapxd-lint`; reserva `npx` para auditorias puntuales.
...
1. Corre el preset completo sobre el proyecto:
   ...
   ```bash npx @skapxd/eslint-opinionated@4 <path> --yes --format toon ```
...
```bash
npx @skapxd/eslint-opinionated@4 <path> --yes --format toon --adopt 10
```
...
4. Verifica el mismo lote:
   ...
   ```bash npx @skapxd/eslint-opinionated@4 <path> --yes --format toon --verify <seed> ```
...
```bash
npx @skapxd/eslint-opinionated@4 <path> --yes --format toon --changed --base origin/main
```
