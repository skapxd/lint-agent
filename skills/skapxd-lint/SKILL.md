...
Ejecuta el paquete publicado, anclado a `@latest`:
...
```bash
npx @skapxd/eslint-opinionated@latest <path> --yes --format toon --adopt 10
```
...
```bash
npx @skapxd/eslint-opinionated@latest <path> --yes --format toon --changed --base origin/main
```
