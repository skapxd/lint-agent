...
Se complementa con `result-error-requires-cause` (preservar la causa) y con `await-requires-result` (obligatoria en los presets tipados: cada `await` resuelve en un `Result`).
