...
Las reglas de swagger del preset (`nest-no-swagger-in-controllers`, `nest-dto-requires-api-property`) descansan sobre una premisa: el plugin `@nestjs/swagger` activo en `nest-cli.json`, que introspecciona DTOs y tipos de retorno. Esta regla **verifica la premisa en vez de asumirla**: anclada al entrypoint (`mainFilePatterns`, default `src/main.ts`, un reporte por proyecto), sube por las carpetas hasta el `nest-cli.json` real y exige:
