...
Impide pasar un `any` invisible como argumento: la frontera debe declararse `unknown` y estrecharse con schema o predicate. Wrapper de typescript-eslint.
...
sendUser(data); // ❌ any invisible como argumento
```
...
sendUser(user); // ✅ argumento validado
```
