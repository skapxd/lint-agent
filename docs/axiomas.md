...
| A4 | **Una unidad, una responsabilidad, un nombre semántico.** El árbol de archivos cuenta una historia; una clase expone una intención. | `one-root-function-per-file`, `filename-matches-root-function`, `max-public-methods`, `no-default-export`, `jsx-return-name-pascal-case`, `max-hook-size`, `no-magic-numbers` |
