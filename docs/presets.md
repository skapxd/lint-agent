...
Las reglas agnosticas al framework viven en `shared.base` como `error`. Por eso `skapxd/nested-function-requires-capture`, que solo depende de AST y scope, `skapxd/no-magic-numbers`, que wrappea una regla sintactica calibrada sin type info, `skapxd/dense-function-requires-comment`, que mide estructura local sin type info, y `skapxd/max-class-size`, que mide clases por AST y rangos físicos, quedan en las bases y las heredan `shared.backend`, `shared.frontend`, `shared.package` y los presets de framework que extienden las bases.
...
- **Una clase = una responsabilidad**: `max-public-methods` (de las reglas base) limita capacidades públicas, recibe los hooks de Nest vía `ignore` y se apaga en `*.controller.ts`/`*.gateway.ts` donde el framework dicta la forma. `max-class-size` sigue activa en esas clases porque el framework no justifica superar 150 líneas. `nest-no-direct-instantiation` exige dependencias por constructor en `*.service.ts`; `nest-no-inline-query-params` consolida 2+ query params en un DTO.
