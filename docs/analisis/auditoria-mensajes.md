...
| `nest-dto-requires-validation` | `missingValidator` | Propiedad DTO sin validador runtime; decora segun tipo, y para uniones/genericos modela con zod/valibot. | - | ya-optimo | Actualizado por #142: ya no distingue input/output por default. |
...
| `prefer-tagged-union-state` | `inconsistentStateShape` | Flags/error independientes permiten combinaciones imposibles; modela union discriminada y consume con `match().exhaustive()`. | - | ya-optimo | Sin cambio. |
| `prefer-tagged-union-state` | `splitStateMachine` | Varios `useState` forman una maquina; usa un `useState` con union discriminada o `useReducer`. | 3 | medio | Anadir criterio: union discriminada directa si el estado es simple; `useReducer` si hay acciones/transiciones repetidas o reglas de transicion concentrables. |
| `prefer-tagged-union-state` | `splitTransition` | Varios setters en una transicion prueban maquina repartida; consolida en union discriminada o `useReducer`. | 3 | medio | Mismo criterio de eleccion union vs reducer que `splitStateMachine`. |
