...
| `await-requires-result` | `shared/backend`, `shared/frontend`, `shared/package`, `nest/base`, `next/server`, `astro/typescript` | `no-ad-hoc-ok-result` | regla | `docs/reglas/no-ad-hoc-ok-result.md:3`: “la exencion type-aware de `await-requires-result` no lo reconoce”; `src/rules/await-requires-result.ts:70-72` exime solo cuando lo awaiteado ya es `Result/Promise<Result>`. | 1 |
